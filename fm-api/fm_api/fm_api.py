...
class FaultAPIs_V2(FaultAPIsBase):
    ...
    def set_fault(self, data):
        ...
        if uuid is None:
            raise ClientException("Failed to execute set_fault.")
        return uuid
    ...
    def clear_fault(self, alarm_id, entity_instance_id):
        ...
        if resp is False:
            raise ClientException("Failed to execute clear_fault.")
    ...
    def get_fault(self, alarm_id, entity_instance_id):
        ...
        resp = fm_core.get(buff)
        if resp is False:
            raise ClientException("Failed to execute get_fault.")
        else:
    ...
    def clear_all(self, entity_instance_id):
        ...
        if resp is False:
            raise ClientException("Failed to execute clear_all.")

    def get_faults(self, entity_instance_id):
        resp = fm_core.get_by_eid(entity_instance_id)
        if resp is False:
            raise ClientException("Failed to execute get_faults.")
        elif resp:
            data = []
            for i in resp:
                data.append(self._str_to_alarm(i))
    ...
    def get_faults_by_id(self, alarm_id):
        resp = fm_core.get_by_aid(alarm_id)
        if resp is False:
            raise ClientException("Failed to execute get_faults_by_id.")
        elif resp:
            data = []
            for i in resp:
                data.append(self._str_to_alarm(i))
